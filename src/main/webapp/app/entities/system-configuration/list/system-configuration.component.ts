...
export class SystemConfigurationComponent implements OnInit {
  ...
  loadAll(): void {
    ...
    this.systemConfigurationService
      ...
      .subscribe({
        next: (res: HttpResponse<ISystemConfiguration[]>) => {
          this.isLoading = false;
          this.paginateSystemConfigurations(res.body, res.headers);
        },
        error: () => {
          this.isLoading = false;
        },
  ...
  trackId(_index: number, item: ISystemConfiguration): number {
    return item.id!;
  }

  delete(systemConfiguration: ISystemConfiguration): void {
    const modalRef = this.modalService.open(SystemConfigurationDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected paginateSystemConfigurations(data: ISystemConfiguration[] | null, headers: HttpHeaders): void {
    ...
    if (data) {
      for (const d of data) {
        this.systemConfigurations.push(d);
      }
    }
  }
}
