...
public interface TeamRepository extends TeamRepositoryWithBagRelationships, JpaRepository<Team, Long> {
    ...
    @Query(
        value = "select distinct team from Team team left join fetch team.preferredFormation",
        countQuery = "select count(distinct team) from Team team"
    )
    Page<Team> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct team from Team team left join fetch team.preferredFormation")
    List<Team> findAllWithToOneRelationships();

    @Query("select team from Team team left join fetch team.preferredFormation where team.id =:id")
